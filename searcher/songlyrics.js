...
export function getLyrics(meta, man) {
	const Clean = (text) => text
		...
		.replace(/@/g, 'at')
		.replace(/&/g, 'and')
		.replace(/ /g, '-'); // SongLyrics formatting

	const artist = Clean(meta.artist);
	const title = Clean(meta.title);
	const url = `https://www.songlyrics.com/${artist}/${title}-lyrics/`;
...
function findLyrics(rootElement) {
	...
	for (const attribute of attributes) {
		if (attribute.key === 'id' && attribute.value === 'songLyricsDiv') {
...
function parseLyrics(element, lyricText) {
	...
	if (tag === 'br') { // SongLyrics formatting
